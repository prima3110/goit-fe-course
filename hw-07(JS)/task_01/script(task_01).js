...
// напиши функции которые с помощью перебирающих методов массивов (никаких for, splice и т. д.) выполняют следующие операции.
...
const getTotalBalance = users => users.reduce((acc, el) => acc + el.balance, 0);
...
const getUniqueSkills = (users) => users.reduce((allSkills, el) => { // Функция, которая пушит все скиллы в новый массив
allSkills.push(...el.skills);
return allSkills;
  }, []);
console.log('all skills:', getUniqueSkills(users));

const foundedSkills = getUniqueSkills(users); // Выносим callback-функцию отдельно, а в reducе передадим ссылку на нее

const myUniqueSkills = (acc, mySkill) => { // Функция, которая пушит уникальные вещи
  if(!acc.includes(mySkill)){
    acc.push(mySkill);
  }
  return acc;
};

const countSkills = (foundedSkills) => foundedSkills.reduce(myUniqueSkills, []).sort(); // Из всех скиллов выбираем уникальные и пушим в новый массив
console.log('unique and sorted skills:', countSkills(foundedSkills));
